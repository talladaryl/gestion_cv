...
$po =$_SESSION['mailto'];
                $q =  "SELECT `idan`, `usnom`, `gategoriea`, `descripa`, `villa`, `titrea`, `datar`, `type`, `nature`, `imgf` FROM `annoncea`  WHERE gategoriea ='Véhicules - Motos - Auto - Camions'  ORDER BY `annoncea`.`idan` DESC ";
              $r = $db -> query($q) ;
              while ($co = $r -> fetch())
              {
              ?> 
